      ...
      <div className="bg-white rounded-2xl shadow-card overflow-hidden h-full">
        ...
        <div className="px-4 py-3 bg-gradient-to-r from-red-500 to-rose-600 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-white font-semibold text-sm uppercase tracking-wide">Expenses</span>
          </div>
          <button
        ...
        <div className="p-3">
          {expenseTransactions.length > 0 ? (
                  ...
                  <div className="flex items-center gap-2 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-gray-800 truncate">
                        {tx.name || tx.payee || categoryDisplay.name || 'Expense'}
                      </p>
                      <p className="text-xs text-gray-500 flex items-center gap-1">
                        ...
                        {tx.source_account_id && (
                          <>
                            ...
                            <span>{getAccountName(tx.source_account_id)}</span>
                          </>
                        )}
                      </p>
                    </div>
            ...
            <div className="py-8 text-center">
              ...
              <button
                onClick={() => setIsModalOpen(true)}
                className="mt-2 text-sm text-red-600 active:text-red-700 hover:text-red-700 font-medium py-2 px-4 rounded touch-manipulation"
              >
                Add expense
              </button>
            </div>
